...
当前提供的截图足以确认面板标题“加工”和全部已授权数字模板，因此首张生成图包含 `anchor.processing_title` 以及 13 组字段专属数字模板，共 63 个条目。

六类星石名称、属性文字和其余状态锚点缺少对应的简中样本，暂不写入生产清单，也不以韩服文字图片替代。后续获得每种状态的国服截图后，只需新增静态 PNG 和清单条目，再运行生成命令；运行时接口无需变更。
